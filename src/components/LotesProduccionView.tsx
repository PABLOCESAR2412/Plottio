...
export function LotesProduccionView() {
  ...
  const lotes = useQuery(api.lotesProduccion.getLotes, {});
  const placas = useQuery(api.placasStock.getTodasPlacasStock, {});
  ...
  const filteredLotes = lotes.filter((l: any) => 
    ...
    (l.notas && l.notas.toLowerCase().includes(deferredSearchTerm.toLowerCase()))
  );

  const filteredPlacas = placas ? placas.filter((p: any) => 
    p.contenido_texto?.toLowerCase().includes(deferredSearchTerm.toLowerCase()) ||
    p.lote_numero?.toLowerCase().includes(deferredSearchTerm.toLowerCase()) ||
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ notas: "", cantidad: 50, material: "acrilico", ancho_cm: 40, alto_cm: 20, texto_base: "Ruta Genérica" });
    } catch (err: any) {
      ...
      <div className="bg-card rounded-2xl shadow-sm border border-border overflow-hidden">
        <div className="p-4 border-b border-border flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder={activeTab === "lotes" ? "Buscar lote o nota..." : "Buscar placa, estado o lote..."}
              value={searchTerm}
        ...
        <div className="overflow-x-auto">
                ...
                <tr key={l._id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      l.estado === 'Terminado' ? 'bg-green-100 text-green-800' : 
                      l.estado === 'En Producción' ? 'bg-blue-100 text-blue-800' :
                      'bg-orange-100 text-orange-800'
                    }`}>
                      {l.estado}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-muted-foreground">{l.notas || "-"}</td>
                  <td className="px-6 py-4 text-muted-foreground">{new Date(l.fechaCreacion).toLocaleDateString()}</td>
                </tr>
              ))}
          ...
          <table className="w-full text-left text-sm text-foreground">
            ...
            <tbody className="divide-y divide-border">
              {filteredPlacas.map((p: any) => (
                <tr key={p._id} className="hover:bg-muted/50 transition-colors">
                  <td className="px-6 py-4">
                    <span className="font-bold text-foreground">{p.contenido_texto || "Sin texto"}</span>
                  </td>
                  <td className="px-6 py-4 text-muted-foreground">
                    <span className="font-mono text-xs px-2 py-1 bg-secondary rounded">{p.lote_numero || "N/A"}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      p.estado === 'Disponible' ? 'bg-green-100 text-green-800' : 
                      p.estado === 'Instalada' ? 'bg-blue-100 text-blue-800' :
                      p.estado === 'Asignada' ? 'bg-purple-100 text-purple-800' :
                      'bg-red-100 text-red-800'
                    }`}>
                      {p.estado}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-muted-foreground capitalize">{p.material}</td>
                  <td className="px-6 py-4 text-muted-foreground">{p.ancho_cm}x{p.alto_cm} cm</td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-border">
              <h3 className="text-xl font-bold text-foreground">Producir Nuevo Lote</h3>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Notas del Lote</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Ancho (cm)</label>
                  <input
                    required
                    type="number"
                    value={formData.ancho_cm}
                    onChange={e => setFormData({...formData, ancho_cm: parseInt(e.target.value)})}
                    className="w-full px-4 py-2 rounded-xl border border-border bg-background focus:ring-2 focus:ring-ring outline-none text-foreground"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Alto (cm)</label>
                  <input
                    required
                    type="number"
                    value={formData.alto_cm}
                    onChange={e => setFormData({...formData, alto_cm: parseInt(e.target.value)})}
                    className="w-full px-4 py-2 rounded-xl border border-border bg-background focus:ring-2 focus:ring-ring outline-none text-foreground"
                  />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Texto Base (Prefijo)</label>
                <input
                  required
                  type="text"
