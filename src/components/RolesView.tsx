...
import { Check, PlusCircle, Server, Settings2, Shield, X } from "lucide-react";
...
export function RolesView() {
	...
	const loadRolePermissions = useQuery(
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			toast.error((err as Error).message || "Error al guardar el rol");
		}
	};
				...
				{(roles ?? []).map((rol) => (
					...
					>
						<div className="flex justify-between items-start mb-4">
							...
							<span
								className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${rol.activo ? "bg-green-500/10 text-green-500" : "bg-destructive/10 text-destructive"}`}
							>
							...
							<form id="roleForm" onSubmit={handleSubmit} className="space-y-8">
								...
								<div>
									<h4 className="font-bold text-foreground mb-4 border-b border-border pb-2 flex items-center gap-2">
										<Server className="w-4 h-4 text-primary" /> Permisos del
										Sistema
									</h4>
									<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
										{Object.entries(permisosPorModulo).map(
											([modulo, perms]) => (
												<div
													key={modulo}
													className="space-y-3 bg-muted/20 p-4 rounded-xl border border-border"
												>
													...
													<div className="space-y-2">
														{perms.map((p) => {
															const isChecked = formData.permisosIds.includes(
																p._id,
															);
															return (
																<label
																	key={p._id}
																	className="flex items-start gap-3 p-2 rounded-lg hover:bg-muted/50 cursor-pointer transition-colors border border-transparent hover:border-border"
																>
																	<div
																		className={`mt-0.5 flex shrink-0 items-center justify-center w-5 h-5 rounded border ${isChecked ? "bg-primary border-primary text-primary-foreground" : "border-input bg-background"}`}
																	>
																		{isChecked && (
