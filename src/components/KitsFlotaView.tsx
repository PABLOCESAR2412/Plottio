...
export function KitsFlotaView() {
  ...
  const generarMasivo = useMutation(api.kitsFlota.generarMasivo);
  ...
  const addItemToKit = () => {
    ...
    const srv = servicios.find((s: any) => s._id === selectedServicioId);
    if (!srv) return;
    ...
    try {
      ...
      await crearKit({
        ...
        items: formData.items.map(item => ({
          servicioId: item.servicioId as any,
      ...
      toast.error(err.message || "Error al crear kit");
    }
  };
  ...
  const handleGenerarMasivoSubmit = async (e: React.FormEvent) => {
    ...
    const vehiculosValidos = generarFormData.vehiculos.filter(v => v.descripcion_vehiculo.trim() !== "");
    ...
    try {
      ...
      toast.success(`Se generaron las cotizaciones para ${vehiculosValidos.length} vehículos exitosamente`);
      setShowGenerarModal(false);
      setGenerarFormData({ modo: "padre_con_subgrupos", vehiculos: [{ descripcion_vehiculo: "" }] });
      setSelectedKit(null);
    } catch (err: any) {
      toast.error(err.message || "Error al generar cotizaciones masivas");
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredKits.map((k: any) => (
          <div key={k._id} className="bg-card rounded-2xl shadow-sm border border-border p-6 hover:shadow-md transition-shadow">
            ...
            <h3 className="text-lg font-bold text-foreground mb-2">{k.nombre}</h3>
            <p className="text-sm text-muted-foreground mb-4 line-clamp-2">
              {k.descripcion || "Sin descripción"}
            </p>
            <div className="border-t border-border pt-4 flex justify-between items-center">
              <span className="text-xs text-muted-foreground">Creado: {new Date(k.fechaCreacion).toLocaleDateString()}</span>
              <button 
                onClick={() => {
                  setSelectedKit(k);
        ...
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-card border border-border rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-border">
              <h3 className="text-xl font-bold text-foreground">Diseñar Kit de Flota</h3>
            </div>
            
            <div className="overflow-y-auto p-6 flex-1 space-y-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Nombre del Kit</label>
                  <input
                    required
                    type="text"
              ...
              <div className="border-t border-border pt-6">
                <h4 className="font-semibold text-foreground mb-4">Ítems del Kit</h4>
                ...
                <div className="flex gap-2 mb-4">
                  ...
                  >
                    <option value="">Seleccionar un servicio del catálogo...</option>
                    {servicios && servicios.map((s: any) => (
                      <option key={s._id} value={s._id}>{s.nombre} - ${s.precioBase}</option>
                    ))}
                  </select>
                  <button 
                    type="button" 
                ...
                <div className="space-y-3">
                  {formData.items.map((item, index) => (
                    <div key={index} className="flex flex-col sm:flex-row gap-3 items-center bg-background p-3 rounded-xl border border-border">
                      ...
                      <div className="w-24">
                        <input
                          ...
                          onChange={(e) => updateItem(index, 'cantidad_por_unidad', parseInt(e.target.value))}
                  ...
                  {formData.items.length === 0 && (
                    <p className="text-sm text-center text-muted-foreground py-4">No hay ítems en este kit todavía.</p>
                  )}
                </div>
              </div>
            </div>
      ...
      {showGenerarModal && selectedKit && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-card border border-border rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-border">
              <h3 className="text-xl font-bold text-foreground">Generar Órdenes: {selectedKit.nombre}</h3>
              <p className="text-sm text-muted-foreground mt-1">Aplica este kit a múltiples vehículos de una vez.</p>
            </div>
            
            <div className="overflow-y-auto p-6 flex-1 space-y-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Modo de Generación</label>
                  <select 
                    value={generarFormData.modo}
                    onChange={(e) => setGenerarFormData({...generarFormData, modo: e.target.value})}
                    className="w-full px-4 py-2 rounded-xl border border-border bg-background focus:ring-2 focus:ring-ring outline-none text-foreground"
                  >
                    <option value="padre_con_subgrupos">Unir todo en 1 Cotización (Vehículos como Subgrupos)</option>
                    <option value="independientes">Crear {generarFormData.vehiculos.filter(v => v.descripcion_vehiculo.trim() !== "").length || 'N'} Cotizaciones Separadas</option>
                  </select>
                </div>

                <div className="border-t border-border pt-4">
                  <label className="block text-sm font-medium text-foreground mb-2">Lista de Vehículos / Placas</label>
                  <div className="space-y-2">
                      ...
                      <div key={idx} className="flex gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            newVehiculos[idx].descripcion_vehiculo = e.target.value;
                            setGenerarFormData({...generarFormData, vehiculos: newVehiculos});
                          }}
                          className="flex-1 px-4 py-2 rounded-xl border border-border bg-background focus:ring-2 focus:ring-ring outline-none text-foreground"
                        />
                        <button
                    ...
                    <button
                      type="button"
                      onClick={() => setGenerarFormData({
                        ...generarFormData, 
                        vehiculos: [...generarFormData.vehiculos, { descripcion_vehiculo: "" }]
                      })}
