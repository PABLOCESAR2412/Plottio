...
export function CatalogoView() {
  ...
  const handleToggle = async (id: Id<"catalogoServicios">, actual: boolean) => {
    try {
      await toggleActivo({ id, activo: !actual });
            ...
            <tbody className="divide-y divide-border">
              {filteredServicios.map((s) => (
                <tr key={s._id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium">${s.precioBase.toFixed(2)}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${s.activo ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-destructive/10 text-destructive'}`}>
